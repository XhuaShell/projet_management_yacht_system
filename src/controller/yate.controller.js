...
export const patch = async (req, res) => {
    ...
    try {
        ...
        if (eslora !== undefined && eslora !== "") {
            ...
            if (isNaN(esloraNum))
                return res.status(400).send("La 200eslora debe ser un número");
        ...
        if (manga !== undefined && manga !== "") {
            ...
            if (isNaN(mangaNum))
                return res.status(400).send("La manga debe ser un número");
            data.manga = mangaNum;
        }
        ...
        if (calado !== undefined && calado !== "") {
            ...
            if (isNaN(caladoNum))
                return res.status(400).send("El calado debe ser un número");
            data.calado = caladoNum;
        }

        if (usuario_dueno_cedula)
            data.usuario_dueno_cedula = usuario_dueno_cedula;
